...
const EditNotes = ({updateNote, setShow}) => {
    ...
    const EditNote = async (e) => {
        ...
        axios.put(`http://localhost:8080/note/updateNote/${updateNote?._id}`, noteData, {
          headers: {
    ...
    useEffect(()=>{
      ...
      setAuthor(updateNote?.author)
      setDescription(updateNote?.description)
      setDateTime(updateNote?.dateTime)
      setExpired(updateNote?.expired)
      setStatus(updateNote?.status) 
    },[updateNote])
  return (
    <div className='flex w-full mt-8 justify-center bg-slate-200'>
      <div className='w-[50%] h-fit'>
      <h1 className='mb-5 font-bold text-2xl'>Add Note</h1>
        <form className='flex flex-col gap-4' onSubmit={EditNote}>
        <label>
          Title:
        ...
        <input type="submit" className='border border-blue-500 font-bold py-2 cursor-pointer rounded-sm mt-5 hover:text-white hover:bg-blue-500' value="Edit Note" />
        </form>
      </div>
    </div>
  )
